...
int main(int argc, char** argv)
{
    ...
    std::string int_db = std::string(DATA_PATH) + "/gaussians_2d-train.dat";
    ...
    app.add_option("--batchsize", batchSize, "BatchSize for integration")->capture_default_str();
    app.add_option("--db", int_db, "Integrand database file path")->capture_default_str();
    ...
    IntegrationTest<GaussiansIntegrands> test(N, D, int_db.c_str(), seed + 1);
    auto loss = [&](const PointArray& array, PointArray& grad){
	    return test.NextBatch(batchSize)(array, grad);
    };
